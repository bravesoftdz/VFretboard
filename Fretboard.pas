...
begin
	...
	for y:=1 to 6 do begin
		...
		for x:=1 to fretCount do begin
			rect.left := (x-1)*fretWidth;
			rect.right := x*fretWidth;
			Canvas.Rectangle(rect);
			if note in FFilter then begin
...
begin
	Result := [Key];
	note := Key;
	for i:=0 to Length(tones)-1 do begin
...
begin
	if Value <> FFretCount then begin
		FFretCount := Value;
		Invalidate;
	end;
end;
...
begin
	if Value <> FFirstFret then begin
		FFirstFret := Value;
		Invalidate;
	end;
end;
